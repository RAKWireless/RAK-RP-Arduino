...
//            dma_channel_set_irq0_enabled(_channelDMA[i], false);
			dma_channel_set_irq1_enabled(_channelDMA[i], false);
        ...
        if (!__channelCount) {
            ...
            irq_remove_handler(DMA_IRQ_1, _irq);
        }
    }
}
...
//        dma_channel_set_irq0_enabled(_channelDMA[i], true);
		dma_channel_set_irq1_enabled(_channelDMA[i], true);

        __channelMap[_channelDMA[i]] = this;
        __channelCount++;
    }
    if (needSetIRQ) {
        irq_add_shared_handler(DMA_IRQ_1, _irq, PICO_SHARED_IRQ_HANDLER_DEFAULT_ORDER_PRIORITY);
////		irq_add_shared_handler(DMA_IRQ_1, _irq, 64);
...
//    dma_channel_acknowledge_irq0(channel);//dma_channel_set_irq0_enabled(channel,true); //dma_channel_acknowledge_irq0(channel);
...
//        if (dma_channel_get_irq0_status(i) && __channelMap[i]) {
		if (dma_channel_get_irq1_status(i) && __channelMap[i]) {
