...
bool I2S::setFrequency(int newFreq) {
    ...
    if (_running) {
      ...
      uint32_t system_clock_frequency = clock_get_hz(clk_sys);
        float bitClk = _freq * _bps * 4.0;
//        Serial.printf("_bps:%d  bitClk:%f\r\n",_bps,_freq,bitClk);
//        pio_sm_set_clkdiv(_pio, _sm, (float)system_clock_frequency / bitClk);
        pio_sm_set_clkdiv(_pio, _sm, (float)clock_get_hz(clk_sys) / bitClk);

//    Serial.printf("sys clock:%d Hz   set pio clock:%d Hz\r\n",system_clock_frequency,_freq);
    }
...
bool I2S::begin() {
	...
	if (_isOutput) {
		offset = pio_add_program(_pio, &pio_i2s_out_program);
//        pio_i2s_out_program_init(_pio, _sm, offset, _pinDOUT, _pinBCLK, _bps);
    } else {
		offset = pio_add_program(_pio, &pio_i2s_in_program);
//        pio_i2s_in_program_init(_pio, _sm, offset, _pinDOUT, _pinBCLK, _bps);
    }
...
//    Serial.printf("_pinDOUT:%d  _pinBCLK:%d  _bps:%d\r\n",_pinDOUT,_pinBCLK,_bps);
    setFrequency(_freq);
    if (_bps == 8) {
    ...
    bool bf_state = _arb->begin(pio_get_dreq(_pio, _sm, _isOutput), _isOutput ? &_pio->txf[_sm] : (volatile void*)&_pio->rxf[_sm]);
//    Serial.printf("_sm:%d  _isOutput:%d  bf_state:%d\r\n",_sm,bf_state,_isOutput);
    _arb->setCallback(_cb);
...
//	if (_isOutput) {
//		 pio_remove_program(_pio, &pio_i2s_out_program,offset);
//       
//    } else {
//		 pio_remove_program(_pio, &pio_i2s_in_program,offset);
//       
//    }
}
